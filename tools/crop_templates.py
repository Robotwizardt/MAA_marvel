...
def crop_image(
    ...
    if x + width > EXPECTED_SCREEN_SIZE[0] or y + height > EXPECTED_SCREEN_SIZE[1]:
        raise ValueError(f"crop box is outside 720x1280: {box}")

    with Image.open(source) as image:
        if image.size != EXPECTED_SCREEN_SIZE:
            raise ValueError(
                f"{source} must be 720x1280, got {image.size[0]}x{image.size[1]}"
            )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Crop MAA templates from 720x1280 fixtures")
