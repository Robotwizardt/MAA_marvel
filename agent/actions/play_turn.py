...
from agent.session.config import LaneOrder, PlayStrategy
...
def _wait_for_agatha(
    ...
    while time.monotonic() < deadline:
        ...
        if performance is not None:
            performance.record(
                "agatha.end_turn_recognition",
                time.perf_counter() - recognition_started,
            )
        if _recognition_hit(matched):
            return True
        waiter.wait_if_static(
            image,
            trace=performance,
...
class PlayTurn(CustomAction):
    ...
    def _run(
        ...
        if state.config.play_strategy is PlayStrategy.AGATHA:
            ready = _wait_for_agatha(context, controller, performance)
            if ready:
                state.allow_end_turn("agatha_ready")
                performance.event("end_turn_gate", allowed=True, reason="agatha_ready")
                return True
            performance.event("end_turn_gate", allowed=False, reason="agatha_timeout")
